...
extern void update_barre_depression(SDL_Renderer *renderer, SDL_Rect *barre_depression, SDL_Rect *barre_sonore, int score){
  ...
  if((*barre_sonore).h == -248 && ((*barre_depression).h-score/2) > -248 && score > 0){
    (*barre_depression).h = (*barre_depression).h - score/2;
  }
  else if(((*barre_depression).h-score/2) <= -248 && score > 0){
    (*barre_depression).h = -248;
  }
  else if(score < 0){
    (*barre_depression).y = (*barre_depression).y + (*barre_depression).h;
    if((*barre_depression).h-score >= -5){
      (*barre_depression).h = 264 - (*barre_depression).y - 5;
    }
    else{
      tempo = (*barre_depression).h;
      (*barre_depression).h = -score;
    }
  }

  if(score >= 0 || (*barre_depression).h == -5){
    if(SDL_SetRenderDrawColor(renderer, 240, 130, 5, SDL_ALPHA_OPAQUE) != 0)
      SDL_ExitWithError("Impossible de changer la couleur du rendu");
  }
  else{
  ...
  if(score < 0 ){
    if(tempo-score >= -5){
      (*barre_depression).h = -5;
    }
    else{
      (*barre_depression).h = tempo - score;
    }
    (*barre_depression).y = 264;
  }
}
